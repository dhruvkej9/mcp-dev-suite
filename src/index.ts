...
server.setRequestHandler(CallToolRequestSchema, async (request) => {
  ...
  try {
    switch (name) {
      case "fs_search": {
        const { directory, pattern, contentRegex } = args as any;
        ...
        if (contentRegex) {
          ...
          results = filtered.filter((f): f is string => f !== null);
        }
      ...
      case "fs_replace": {
        const { directory, pattern, find, replace, dryRun } = args as any;
        const files = await globby(pattern, { cwd: directory, absolute: true });
      ...
      case "db_sqlite_query": {
        const { dbPath, query, params = [] } = args as any;
